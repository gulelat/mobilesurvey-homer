		...
		if (mRenderer != null && action == MotionEvent.ACTION_MOVE) {
			if (oldX >= 0 || oldY >= 0) {
				...
				if (event.getPointerCount() > 1
					...
					float newDeltaX = Math
							.abs(newX - newX2);
					float newDeltaY = Math
							.abs(newY - newY2);
					float oldDeltaX = Math
							.abs(oldX - oldX2);
					float oldDeltaY = Math
							.abs(oldY - oldY2);
					float zoomRate = 1;

					float tan1 = Math.abs(newY - oldY)
							/ Math.abs(newX - oldX);
					...
					if (tan1 <= 0.577 && tan2 <= 0.577) {
						...
						if (zoomRate > 0.909
							...
							&& tan2 >= 1.732) {
						// pinch zoom vertically,
						// |deltaY| / |deltaX| is [1.732
						// ~ infinity], 1.732 is the
						// approximate value of
						// tan(Pi/3)
						zoomRate = newDeltaY
								/ oldDeltaY;
					...
					} else if ((tan1 > 0.577 && tan1 < 1.732)
							&& (tan2 > 0.577 && tan2 < 1.732)) {
						// pinch zoom diagonally
						if (Math.abs(newX - oldX) >= Math
								.abs(newY
										- oldY)) {
							zoomRate = newDeltaX
									/ oldDeltaX;
			...
			if (mRenderer != null && mRenderer.isZoomEnabled()
				...
				} else if (oldX < zoomR.left + zoomR.width()
						* 2 / 3) {
