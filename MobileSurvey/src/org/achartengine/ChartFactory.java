...
public class ChartFactory {
	 ...
	 *                 if dataset is null or renderer is null or if the
	 *                 dataset and the renderer don't include the same
	 *                 number of series
	 */
	...
	public static final GraphicalView getCubeLineChartView(Context context,
		...
		checkParameters(dataset, renderer);
		XYChart chart = new CubicLineChart(dataset, renderer,
				smoothness);
		return new GraphicalView(context, chart);
	}
	 ...
	 *                the multiple series renderer (cannot be null)
	 * @param format
	 *                the date format pattern to be used for displaying the
	 *                X axis date labels. If null, a default appropriate
	 *                format will be used.
	 ...
	 * @throws IllegalArgumentException
	 *                 if dataset is null or renderer is null or if a
	 *                 dataset number of items is different than the number
	 *                 of series renderers or number of chart types
	 */
	...
	/**
	 * Creates a pie chart intent that can be used to start the graphical
	 * view activity.
	 * 
	 * @param context
	 *                the context
	...
	/**
	 * Creates a dial chart intent that can be used to start the graphical
	 * view activity.
	 * 
	 * @param context
	 *                the context
	...
	/**
	 * Creates a doughnut chart intent that can be used to start the
	...
	/**
	 * 
	 * Creates a line chart intent that can be used to start the graphical
	 * view activity.
	 * 
	 * @param context
	 *                the context
	...
	/**
	 * 
	 * Creates a cubic line chart intent that can be used to start the
	 * graphical view activity.
	 * 
	 * @param context
	 *                the context
	...
	public static final Intent getCubicLineChartIntent(Context context,
			XYMultipleSeriesDataset dataset,
			XYMultipleSeriesRenderer renderer, float smoothness) {
		return getCubicLineChartIntent(context, dataset, renderer,
				smoothness, "");
	}

	/**
	 * Creates a scatter chart intent that can be used to start the
	 * graphical view activity.
	 * 
	 * @param context
	 *                the context
	...
	/**
	 * Creates a time chart intent that can be used to start the graphical
	 * view activity.
	 * 
	 * @param context
	 *                the context
	...
	/**
	 * Creates a bar chart intent that can be used to start the graphical
	 * view activity.
	 * 
	 * @param context
	 *                the context
	 ...
	 *                then the title bar will be hidden. If a blank title is
	 *                passed in, then the title bar will be the default.
	 *                Pass in any other string to set a custom title.
	 * @return a line chart intent
	 * @throws IllegalArgumentException
	 *                 if dataset is null or renderer is null or if the
	 *                 dataset and the renderer don't include the same
	...
	/**
	 * Creates a range bar chart intent that can be used to start the
	 * graphical view activity.
	 * 
	 * @param context
	 *                the context
	...
	/**
	 * Creates a combined XY chart intent that can be used to start the
	 * graphical view activity.
	 * 
	 * @param context
	 *                the context
	...
	public static final Intent getDialChartIntent(Context context,
			CategorySeries dataset, DialRenderer renderer,
			String activityTitle) {
		checkParameters(dataset, renderer);
		Intent intent = new Intent(context, GraphicalActivity.class);
		DialChart chart = new DialChart(dataset, renderer);
	 ...
	 *                 of series renderers
	 */
	private static void checkParameters(MultipleCategorySeries dataset,
			DefaultRenderer renderer) {
		if (dataset == null
				|| renderer == null
	...
	private static boolean checkMultipleSeriesItems(
			MultipleCategorySeries dataset, int value) {
		int count = dataset.getCategoriesCount();
		boolean equal = true;
		for (int k = 0; k < count && equal; k++) {
			equal = dataset.getValues(k).length == dataset
					.getTitles(k).length;
		}
		return equal;
	}
