...
public abstract class AbstractChart implements Serializable {
	...
	private static float[] calculateDrawPoints(float p1x, float p1y, float p2x, float p2y, int screenHeight, int screenWidth) {
		...
		if (p1y > screenHeight) {
			...
			if (drawP1x < 0) {
				// If Intersection is left of the screen we
				// calculate the intersection
				// with the left border
				drawP1x = 0;
				drawP1y = p1y - m * p1x;
			} else if (drawP1x > screenWidth) {
				// If Intersection is right of the screen we
				// calculate the intersection
	 ...
	 *                the paint
	 */
	protected void drawLabel(Canvas canvas, String labelText,
			DefaultRenderer renderer, List<RectF> prevLabelsBounds,
			int centerX, int centerY, float shortRadius,
			float longRadius, float currentAngle, float angle,
			int left, int right, int color, Paint paint) {
	...
	/**
	 * Given screen coordinates, returns the series and point indexes of a
	 * chart element. If there is no chart element (line, point, bar, etc)
	 * at those coordinates, null is returned.
	 * 
	 * @param screenPoint
	 * @return
	 */
