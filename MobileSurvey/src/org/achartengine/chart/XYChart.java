...
public abstract class XYChart extends AbstractChart {
	...
	/**
	 * The clickable areas for all points. The array index is the series
	 * index, and the RectF list index is the point index in that series.
	 */
	...
	public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
		...
		String[] titles = new String[sLength];					// title
		...
		// 2) We don't need random seeking, only sequential
		// reading/writing, so
		...
		for (int i = 0; i < sLength; i++) {
			...
			for (Entry<Double, Double> value : range.entrySet()) {
				...
				// points.add((float) (left +
				// xPixelsPerUnit[scale]
				// * (value.getKey().floatValue() -
				// minX[scale])));
				// points.add((float) (bottom -
				// yPixelsPerUnit[scale]
		...
		// draw stuff over the margins such as data doesn't render on
		// these areas
		...
		if (showLabels || showGridX) {
			List<Double> xLabels = getValidLabels(getXLabels(
					minX[0], maxX[0],
					mRenderer.getXLabels()));
			Map<Integer, List<Double>> allYLabels = getYLabels(
					minY, maxY, maxScaleNumber);
			...
			if (showLabels) {
				...
				if (mRenderer.getXLabelsAlign() == Align.LEFT) {
					xLabelsLeft += mRenderer
							.getLabelsTextSize() / 4;
				}
			}
			drawXLabels(xLabels,
					mRenderer.getXTextLabelLocations(),
					canvas, paint, xLabelsLeft, top,
					bottom, xPixelsPerUnit[0], minX[0],
					maxX[0]);
			drawYLabels(allYLabels, canvas, paint, maxScaleNumber,
					left, right, bottom, yPixelsPerUnit,
					minY);

			if (showLabels) {
				paint.setColor(mRenderer.getLabelsColor());
				for (int i = 0; i < maxScaleNumber; i++) {
					Align axisAlign = mRenderer
							.getYAxisAlign(i);
					...
					for (Double location : yTextLabelLocations) {
						if (minY[i] <= location
							...
							String label = mRenderer
									.getYTextLabel(location,
											i);
							paint.setColor(mRenderer
									.getYLabelsColor(i));
							paint.setTextAlign(mRenderer
									.getYLabelsAlign(i));
							if (or == Orientation.HORIZONTAL) {
			...
			if (showLabels) {
				...
				if (or == Orientation.HORIZONTAL) {
					...
					paint.setTextSize(mRenderer
							.getChartTitleTextSize());
					drawText(canvas,
							mRenderer.getChartTitle(),
							x + width / 2,
							y
									+ mRenderer.getChartTitleTextSize(),
							paint, 0);
				} else if (or == Orientation.VERTICAL) {
					drawText(canvas, mRenderer.getXTitle(),
							x + width / 2,
							y + height - size,
							paint, -90);
					drawText(canvas, mRenderer.getYTitle(),
							right + 20, y + height
									/ 2,
							paint, 0);
					paint.setTextSize(mRenderer
		...
		if (or == Orientation.HORIZONTAL) {
			drawLegend(canvas, mRenderer, titles, left, right, y,
		...
		if (mRenderer.isShowAxes()) {
			...
			if (or == Orientation.HORIZONTAL) {
				...
				if (rightAxis) {
					canvas.drawLine(right, top, right,
							bottom, paint);
				}
			} else if (or == Orientation.VERTICAL) {
	...
	protected Map<Integer, List<Double>> getYLabels(double[] minY,
		...
		for (int i = 0; i < maxScaleNumber; i++) {
			allYLabels.put(i, getValidLabels(MathHelper.getLabels(
					minY[i], maxY[i],
	...
	protected void drawSeries(XYSeries series, Canvas canvas, Paint paint,
			...
			int seriesIndex, Orientation or, int startIndex) {
		...
		if (stroke != null) {
			...
			setStroke(stroke.getCap(), stroke.getJoin(),
					stroke.getMiter(),
		...
		if (seriesRenderer.isDisplayChartValues()) {
			paint.setTextAlign(seriesRenderer
					.getChartValuesTextAlign());
			drawChartValuesText(canvas, series, seriesRenderer,
					paint, points, seriesIndex, startIndex);
		}
		if (stroke != null) {
	...
	protected void drawChartValuesText(Canvas canvas, XYSeries series,
		...
		if (points.length > 1) { // there are more than one point
			...
			for (int k = 0; k < points.length; k += 2) {
				if (k == 2) { // decide whether to display first
						// two points' values or not
					if (Math.abs(points[2] - points[0]) > 100
							|| Math.abs(points[3]
									- points[1]) > 100) {
						// first point
						drawText(canvas,
								getLabel(series.getY(startIndex)),
								points[0],
								points[1]
						...
						// second point
						drawText(canvas,
								getLabel(series.getY(startIndex + 1)),
								points[2],
								points[3]
										- renderer.getChartValuesSpacing(),
								paint, 0);
					...
					// compare current point's position with
					// the previous point's, if they are not
					// too close, display
	...
	protected void drawText(Canvas canvas, String text, float x, float y,
		...
		float angle = -mRenderer.getOrientation().getAngle()
				+ extraAngle;
		if (angle != 0) {
	...
	/**
	 * Transform the canvas such as it can handle both HORIZONTAL and
	...
	protected void drawXLabels(List<Double> xLabels,
		...
		for (int i = 0; i < length; i++) {
			...
			if (showGridY) {
				...
				canvas.drawLine(xLabel, bottom, xLabel, top,
						paint);
			}
		}
	...
	protected void drawYLabels(Map<Integer, List<Double>> allYLabels,
			Canvas canvas, Paint paint, int maxScaleNumber,
		...
		for (int i = 0; i < maxScaleNumber; i++) {
			...
			for (int j = 0; j < length; j++) {
				...
				boolean textLabel = mRenderer.getYTextLabel(
						label, i) != null;
				float yLabel = (float) (bottom - yPixelsPerUnit[i]
						* (label - minY[i]));
				if (or == Orientation.HORIZONTAL) {
	...
	public SeriesSelection getSeriesAndPointForScreenCoordinate(
		...
		if (clickableAreas != null)
			for (int seriesIndex = clickableAreas.size() - 1; seriesIndex >= 0; seriesIndex--) {
				// series 0 is drawn first. Then series 1 is
				// drawn on top, and series 2
				// on top of that.
				// we want to know what the user clicked on, so
				// traverse them in the
	 ...
	 *                the start index of the rendering points
	 */
	protected abstract ClickableArea[] clickableAreasForPoints(
			float[] points, double[] values, float yAxisValue,
